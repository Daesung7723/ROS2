...
CENTER_X = 5.5
CENTER_Y = 5.5
AVOIDANCE_DISTANCE = 1.2 # 회피 거리 살짝 증가
...
class Turtle1ControllerNode(Node):
    def __init__(self):
        ...
        self.turtle2_pose = None # turtle2의 위치 (없으면 None)
        self.publisher_ = None
        self.state = 'wandering' # 'wandering', 'recovering', 'avoiding'
        ...
        # turtle2의 존재와 상관없이 우선 구독 시도
        self.turtle2_subscription = self.create_subscription(
            Pose, '/turtle2/pose', self.turtle2_pose_callback, 10)
        
        timer_period = 0.1
        self.timer_ = self.create_timer(timer_period, self.timer_callback)
        self.get_logger().info(f'"{self.turtle_name}" controller started. Will avoid turtle2 if present.')
    ...
    def check_and_spawn_turtle(self):
        ...
        turtle1_exists = any(f'/{self.turtle_name}/' in name for name, _ in service_names_and_types)
        if turtle1_exists: self.get_logger().info(f'"{self.turtle_name}" already exists.')
        else:
            self.get_logger().info(f'"{self.turtle_name}" not found. Spawning a new one.')
            request = Spawn.Request()
            request.x, request.y, request.theta, request.name = 2.0, 2.0, 0.0, self.turtle_name
    ...
    def timer_callback(self):
        ...
        # --- 1. 상태 결정 로직 (수정된 부분) ---
        is_avoiding = False
        # turtle2의 위치 정보(self.turtle2_pose)가 있을 때만 회피 로직 검사
        if self.turtle2_pose is not None:
            dist_x = self.current_pose.x - self.turtle2_pose.x
            dist_y = self.current_pose.y - self.turtle2_pose.y
        ...
        # --- 2. 상태에 따른 행동 결정 (이전과 동일) ---
        twist_msg = Twist()
        if self.state == 'avoiding':
            self.get_logger().warn('State: Avoiding turtle2!', throttle_duration_sec=1)
            escape_angle = math.atan2(self.current_pose.y - self.turtle2_pose.y, self.current_pose.x - self.turtle2_pose.x)
