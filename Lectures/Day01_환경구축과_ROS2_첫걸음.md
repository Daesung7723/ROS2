...
| 4 | 9/14 | 카메라 연결 — 원격 연결과 카메라 노드 구동 (RPi5 전환) |
| 5 | 9/21 | 영상 처리와 AI 이미지 분류 — 색상 검출·표지판 인식 |
| 6 | 9/28 | 실물 모듈 — 초음파센서·DC모터 |
| — | 10/5 | 휴강 (개천절 대체공휴일) |
| 7 | 10/12 | SLAM 실습 — Gazebo에서 지도 작성과 목표 주행 |
| 8 | 10/19 | 시뮬레이션 통합 — 표지판에 반응하는 turtle |
...
| 층 | 역할 | 예 |
|------|------|------|
| 사용자 | 서비스 이용·피드백 | 가정·산업 현장·연구실의 로봇 사용자 |
| 응용프로그램(서비스) | 로봇이 제공하는 기능 구현 | 자율주행·물류 배송·청소·제조 자동화·안내 로봇 |
| **ROS2 플랫폼** | 통신 체계·개발 도구·패키지 생태계 | 노드/토픽 통신·CLI·rqt·RViz·Gazebo·오픈소스 패키지 |
| 운영체제 | 프로세스·파일·장치 관리 | Ubuntu 등 Linux 배포판·Windows·macOS |
| 하드웨어 | 연산 장치 + 센서·구동부 | PC·SBC(Single Board Computer — Raspberry Pi 등) + 카메라·라이다·모터 |

- 이 과정의 조합 — 하드웨어 **Raspberry Pi 5** / 운영체제 **Ubuntu 24.04** / 플랫폼 **ROS2 Jazzy** / 응용 **자율차(직접 제작)**
- ROS는 전통적 운영체제가 아니라 **기존 OS 위에서 동작**합니다. OS의 프로세스 관리·파일시스템을 그대로 쓰면서, 그 위에 로봇 개발 필수 기능(노드 간 통신·기록/재생·라이브러리·패키지 생태계)을 제공합니다

**컴퓨터 생태계와의 비교** — 컴퓨터 산업은 부품·운영체제·응용프로그램이 분업하는 생태계이며, 응용 개발자는 하드웨어 내부를 알지 못해도 개발할 수 있습니다. 로봇 산업에서 ROS가 같은 구조를 지향합니다.

- 과거 — 로봇마다 통신·제어·도구를 처음부터 개발 (중복 개발·비효율)
- 현재 — 공통 기반은 ROS가 제공하고, 개발자는 응용(서비스)에 집중

**생태계의 현재 규모**

| 지표 | 값 |
|------|------|
| ROS2가 차지하는 비중 | 전체 ROS 내려받기의 **91.2%** (2024년 72%) |
| ROS를 사용하는 기업 | **1,579곳** (전년 대비 26% 증가) |
| 연간 내려받기 | 약 **9.8억 건** (전년 대비 85% 증가) |

- 출처 — Open Robotics 「2025 ROS Metrics Report」(2026-02 발행)
- 1년 사이 72%에서 91.2%로 상승 — ROS1에서 ROS2로의 이주가 마무리 국면임을 뜻합니다(ROS1 Noetic 지원 종료 = 2025-05)
- 국내 채용에서도 ROS2 실무 경험이 요구 역량으로 제시됩니다 — 자율주행 이송로봇·물류 로봇·Physical AI 연구개발 분야

**왜 ROS2를 새로 만들었는가**

- ROS1은 2007년 Willow Garage가 개인 서비스 로봇 PR2용 프레임워크를 오픈소스로 공개한 것이 시작입니다. 널리 보급되었으나 초기 전제(단일 로봇·워크스테이션급 컴퓨터·Linux 전용·실시간 제어 미지원·안정된 네트워크)를 그대로 계승한 제약이 있었습니다
- 산업 현장의 새 요구 — 복수 로봇·임베디드 시스템·실시간 제어·불안정한 네트워크에서의 동작·멀티 플랫폼·상업용 제품 지원
- 이 요구를 반영하려면 대규모 구조 변경이 필요해 **ROS1과 분리해 ROS2를 새로 개발**했습니다 (`ros1_bridge`로 점진적 이전 지원)
- ROS2 설계의 핵심 — 마스터(단일 장애점) 소멸 · DDS 표준 채택 · 생산(양산) 지향 설계 · 미들웨어 벤더 선택 가능 · Apache 2.0 오픈소스
...
**설계 관점 — 왜 기능 하나당 노드 하나인가**

노드 구조는 객체지향 설계 원칙을 여러 프로그램에 걸쳐 적용한 형태입니다. 객체지향의 기본 발상은 데이터와 기능을 한 덩어리로 묶고, 내부 구현은 공개하지 않은 채 정해진 규격으로만 주고받는 것입니다.

| 객체지향 | ROS2 |
|------|------|
| 객체(object) | **노드** |
| 멤버 변수 | 노드 내부 상태 — 센서 값·목표 속도 등 |
| 메서드 호출 | **메시지 교환** (토픽·서비스) |
| 인터페이스(공개 규격) | **토픽 이름 + 메시지 타입** |
| 캡슐화 | 노드 내부 구현은 외부에서 알 수 없음 |

| 지표 | 뜻 | ROS2에서의 실현 |
|------|------|------|
| **응집도(cohesion) — 높게** | 한 단위가 하나의 목적에 집중하는 정도 | 노드 하나 = 기능 하나 — 카메라 노드는 영상만, 모터 노드는 구동만 담당 |
| **결합도(coupling) — 낮게** | 단위 사이의 의존 정도 | 노드가 서로를 직접 호출하지 않음 — 토픽 이름·메시지 타입만 일치하면 연결 |

| 구분 | 직접 함수 호출 | 메시지 교환 |
|------|------|------|
| 상대 파악 | 함수 이름·인자 형식·소속 모듈을 알아야 함 | 토픽 이름·메시지 타입만 알면 됨 |
| 실행 위치 | 같은 프로그램 안 | 다른 프로세스·다른 기기 가능 |
| 상대 부재 시 | 호출 불가 | 발행 자체는 성립 |
| 교체 | 호출부 수정 필요 | 같은 토픽을 쓰면 수정 불요 |

- **응집도를 높이고 결합도를 낮춘다** — 노드 분할과 메시지 교환이 이 원칙의 구현입니다. Day 9에서 구동 노드 하나만 바꿔 시뮬레이션을 실물로 옮길 수 있는 근거가 여기에 있습니다
- Day 2에서 작성하는 rclpy 노드는 실제로 Python 클래스(`Node` 상속)로 만듭니다 — 객체와 노드의 대응이 코드에서 그대로 나타납니다
...
수업에서 다룬 20가지 특징 중 이 과정과 직접 관련된 것을 추립니다.

| 분류 | 특징 |
|------|------|
| 플랫폼 | Linux·Windows·macOS 3대 OS 지원 — 이 과정은 Ubuntu 24.04 |
| 통신 | DDS 미들웨어 채택 — RTPS(Real-Time Publish-Subscribe) 프로토콜·데이터 중심 발행/구독 |
| 노드 발견 | 마스터(roscore) 소멸 — DDS가 노드를 자동 발견해 **단일 장애점 제거** |
| 미들웨어 선택 | RMW(ROS Middleware) 추상화 계층 — DDS 제품 교체 가능(기본 = eProsima Fast DDS) |
| 실시간성 | 전제 충족 시(전용 하드웨어·RTOS) 실시간 제어 지원 |
| 보안 | DDS-Security·SROS 2 — ROS1의 마스터 IP·포트 노출 취약점 해소 |
| 클라이언트 라이브러리 | RCL(ROS Client Library) 위에 언어별 rclcpp·**rclpy**(Day 2부터 사용) |
| 빌드 | ament 빌드 시스템 + **colcon** 빌드 도구(Day 3부터 사용) |
| launch | Python launch 지원(Day 3) — 여러 노드 일괄 기동·복잡한 구성 가능 |
| 임베디드 | DDS-XRCE(eXtremely Resource Constrained Environments) — 자원 제약 보드에서도 동작 |
...
**WSL**(Windows Subsystem for Linux) — Windows 위에서 Linux 실행 파일을 그대로 구동하는 호환 계층입니다. 가상머신과 달리 별도 OS 전체를 구동하지 않아 부담이 적고, **WSL2는 실제 Linux 커널을 탑재**해 대부분의 명령이 그대로 동작합니다.
...
> **Tip —** 오늘은 터미널을 3개까지 사용합니다. `sudo apt install terminator`로 설치하면 창 하나를 분할해 사용할 수 있어 화면 전환이 줄어듭니다(선택).
...
- **커널(kernel)** — 운영체제의 핵심부. 부팅 후 메모리에 상주하며 프로세스·메모리·파일·장치를 관리합니다
- **셸(shell)** — 사용자와 커널 사이의 **명령어 해석기** — 입력한 명령을 해석해 커널에 전달하고, 결과를 사용자에게 출력합니다

```
사용자 ──명령 입력──▶ 셸 ──해석·전달──▶ 커널 ──제어──▶ 하드웨어
   ◀────결과 출력──── 셸
```

동작 순서 — ① 프롬프트(`$`) 표시·입력 대기 → ② 명령 입력·셸이 해석 → ③ 내장 명령은 셸이 직접 실행, 외부 명령은 **자식 프로세스를 생성해 실행** → ④ 종료 후 다시 프롬프트

커널과 셸은 모든 운영체제의 공통 구조이며 이름만 다릅니다:

| 운영체제 | 커널 | 대표 셸 |
|------|------|------|
| Windows | NT 커널 | PowerShell · cmd |
| Linux | Linux 커널 | **bash** · dash · zsh |
| macOS | XNU | zsh |

**ROS2는 어디에 있는가** — ROS2는 커널도 셸도 아니며, **셸에서 실행되는 사용자 프로그램(노드)의 집합**입니다.

| 층 | 내용 |
|------|------|
| 하드웨어 | PC · RPi5 · 센서 · 모터 |
| 커널 | 프로세스·메모리·장치 관리 |
| 셸 | 명령 해석 — `ros2 run ...` 입력을 받는 자리 |
| **ROS2 노드** | 셸이 실행한 **프로세스** — `turtlesim_node`·`turtle_teleop_key` 등 |
| DDS 통신 | 노드 사이의 메시지 전달 (2.5 참조) |

- `ros2` 명령 = 셸이 해석해 실행하는 **외부 명령**입니다
- **`source /opt/ros/jazzy/setup.bash`가 하는 일** = 셸 환경에 ROS2 경로·설정을 등록하는 것. 새 터미널마다 필요한 이유가 여기에 있으며, `~/.bashrc` 등록(3.5)이 이를 자동화합니다

**탐색·조회**

| 명령 | 기능 | 주요 옵션·예 |
|------|------|------|
| `ls` | 현재 디렉터리 목록 조회 | `-a` 숨김 포함 / `-l` 상세 / `-h` 크기 단위 / `-t` 시간순 |
...
| `rm` | 파일·디렉터리 삭제 — **주의** | `-r` 디렉터리 포함 / `-f` 강제 |
...
**프로세스 관리 — 노드 하나 = 프로세스 하나**

- **프로세스(process)** — 실행 중인 프로그램. 프로세스마다 PID(Process ID — 식별 번호)가 부여됩니다
- 셸이 외부 명령을 실행할 때 자식 프로세스를 생성합니다 — `ros2 run ...`도 같은 방식이며, **ROS2 노드가 곧 프로세스**입니다

| 조회 명령 | 기능 |
|------|------|
| `ps -ef` | 전체 프로세스 상세 — PID·부모 PID·시작 시각·명령 |
| `ps -ef \| grep turtlesim` | 이름으로 걸러 조회 |
| `pgrep -a turtlesim` | 이름으로 PID·실행 명령 조회 |
| `top` | 실시간 갱신 — CPU·메모리 점유 확인 (`q`로 종료) |

| 종료 방법 | 신호 | 성격 |
|------|------|------|
| Ctrl+C | SIGINT | **정상 종료 요청** — 프로그램이 마무리 작업 후 종료. 기본 수단 |
| `kill PID` | SIGTERM | 종료 요청 — 다른 터미널에서 PID를 지정 |
| `kill -9 PID` | SIGKILL | **강제 종료** — 마무리 작업 없이 즉시. 최후 수단 |
| `pkill -f turtlesim` | SIGTERM | 이름 패턴으로 일괄 종료 |

같은 대상을 두 관점으로 조회할 수 있습니다:

```bash
ros2 node list                # ROS2 관점 — 등록된 노드 이름
ps -ef | grep turtlesim       # 운영체제 관점 — 실행 중인 프로세스
```

> **자주 하는 실수 —** 터미널 창만 닫고 노드가 남는 경우, 남은 노드가 계속 토픽을 발행하므로 같은 이름의 노드를 다시 실행하면 이름이 충돌하거나 turtle이 명령을 두 번 받습니다. `ros2 node list`에 없어야 할 이름이 보이면 `pkill -f turtlesim`으로 정리합니다. `kill -9`는 곧바로 사용하지 않습니다 — Ctrl+C → `kill` 순으로 시도하고, 반응이 없을 때만 사용합니다.

- `ros2 daemon status` · `ros2 daemon stop` — CLI 조회 결과가 실제와 어긋날 때 데몬(배경에서 상시 동작하는 프로세스)을 재시작하는 수단입니다
- Day 6 이후 재사용 — 실물 차량에서 모터 노드가 남아 있으면 바퀴가 계속 회전하는 상태가 되므로, **종료 확인이 안전 절차의 일부**입니다
...
- Day 4 카메라·Day 6 초음파에서 재사용합니다 — 센서가 예상 주기로 동작하는지 확인하는 첫 진단 수단입니다
...
- Day 6 실물 차량에서도 같은 `Twist` 타입을 사용합니다
