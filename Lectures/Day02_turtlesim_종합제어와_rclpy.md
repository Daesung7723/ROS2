...
1. [오늘의 목표](#1-오늘의-목표)
2. [작업 환경과 메시지 관찰](#2-작업-환경과-메시지-관찰)
...
> **확인되지 않는 경우 —** Day 1에 설정한 환경이 남아 있지 않을 수 있습니다. 증상별로 확인합니다.

| 증상 | 확인·조치 |
|------|------|
| `ros2: command not found` | `source /opt/ros/jazzy/setup.bash` 실행 → 동작하면 `~/.bashrc` 등록이 빠진 것(Day 1 자료 3.5 재수행 · 1분) |
| `/opt/ros/jazzy` 폴더가 없음 | ROS2 설치본이 남아 있지 않은 것 — Day 1 자료 3.4 절차 재수행(20~30분) |
| `wsl -l -v`에 Ubuntu-24.04가 없음 | WSL 배포판이 초기화된 것 — 강사에게 알린 뒤 Day 1 자료 3.2부터 재수행 |

복구 중에도 이론 구간(3장·6장 앞부분)은 함께 진행할 수 있습니다.

---

## 2. 작업 환경과 메시지 관찰

### 2.1 두 가지 도구를 함께 사용합니다

| 도구 | 담당 | 이유 |
|------|------|------|
| **terminator** (분할) | ① 시뮬레이터 · ② **모니터** · ③ **실행·조회 명령** | 수업 내내 유지하는 화면 + 실행 결과를 모니터와 나란히 확인하는 자리 |
| **VS Code** (WSL 연결) | ④ 스크립트 작성·편집 | 파일을 작성하는 자리 — **실행은 터미널이 담당** |

**왜 두 가지인가**

- Linux에서는 터미널을 **여러 개 동시에 쓰는 것이 기본**입니다 — 노드 하나가 터미널 하나를 점유하기 때문입니다
- 접근 경로는 여러 가지입니다 — 기본 터미널 · terminator · VS Code 통합 터미널 · Windows Terminal 탭. **경로가 달라도 결국 같은 bash 셸**입니다
- **Day 4부터는 Raspberry Pi 5에 원격으로 연결**해 작업하며, **Day 6부터는 차량에 장착한 Raspberry Pi 5**가 그 대상입니다. 원격 환경에서는 편집기 없이 터미널만 사용하는 경우가 있으므로, 지금 두 방식을 모두 다루어 둡니다

### 2.2 terminator — 상시 유지 터미널
...
| 분할 | 실행 |
|:--:|------|
| 좌 | `ros2 run turtlesim turtlesim_node` — 종료하지 않고 유지 |
| 우 | **`ros2 topic echo /turtle1/cmd_vel`** — **오늘 내내 유지** |

- 모니터의 목적 — 지금까지는 "명령 → 화면 변화"만 확인했으나, 모니터가 있으면 **명령 → 메시지 → 화면 변화**의 중간 단계가 보입니다
- 진단 수단 — turtle이 움직이지 않을 때 **모니터에 메시지가 출력되는지**로 원인이 둘로 나뉩니다. 출력되면 수신 측 문제, 출력되지 않으면 발행 측 문제입니다. Day 6 실물 차량의 1차 점검도 같은 방식입니다
...
### 2.3 VS Code — WSL 연결과 확장

VS Code는 **Windows에 설치된 것을 그대로 사용**합니다. Ubuntu 안에 따로 설치하지 않습니다 — WSL 확장이 Linux 쪽에 서버를 자동으로 배치하므로 불필요합니다.

```bash
cd ~/ros2_scripts
code .                            # Ubuntu 터미널에서 실행 — WSL에 연결된 창이 열림
```

- 첫 실행 시 VS Code Server가 WSL 안에 자동 설치됩니다(인터넷 필요)
- **좌하단에 `WSL: Ubuntu-24.04` 표시**를 확인합니다. 이 표시가 없으면 Windows 로컬 창이며 `ros2` 명령이 동작하지 않습니다
- 통합 터미널(`Ctrl + ~`)도 같은 bash이지만, **실행은 별도 터미널에서 수행**합니다 — 시뮬레이터·모니터와 나란히 두고 결과를 확인하기 위해서입니다
- 작업 폴더는 `~/ros2_scripts`(Linux 파일시스템)입니다. `/mnt/c/...`(Windows 디스크)에 두면 느리고 권한 문제가 발생합니다

**확장 설치 — 로컬용과 원격용이 분리됩니다**

| 확장 | 설치 위치 | 구분 |
|------|------|:--:|
| **WSL** (Microsoft) | **로컬(Windows)** | 필수 — 없으면 WSL 연결 자체가 성립하지 않음 |
| Python (Microsoft) | 원격(WSL) | 선택 — 자동 완성·문법 검사용 |
| Pylance | 원격(WSL) | 선택 (Python 확장과 동반 설치) |
| YAML (Red Hat) | 원격(WSL) | 선택 — Day 3 launch 파일용 |

- **실행에는 확장이 필요하지 않습니다** — 스크립트를 실행하는 주체는 터미널의 `python3`이며 편집기 설정과 무관합니다
- 확장 탭을 열면 목록이 **`LOCAL - INSTALLED`**와 **`WSL: UBUNTU-24.04 - INSTALLED`** 두 구획으로 나뉩니다
- 자동 완성을 사용하려는 경우, Python 확장이 로컬에만 있으면 원격 구획에 다시 설치합니다 — 확장을 검색한 뒤 **`Install in WSL: Ubuntu-24.04`** 버튼

> **참고 —** `import rclpy`에 빨간 밑줄이 표시되는 경우, 편집기의 표시일 뿐 실행에는 지장이 없습니다. 자동 완성·문법 검사를 사용하려면 ① 창의 WSL 연결 표시(좌하단) ② Python 확장의 원격(WSL) 설치 여부를 확인합니다.

### 2.4 메시지 필드 확인 — 타입 → 구조 → 값
...
| ③ 값 | `ros2 topic echo <토픽>` | 지금 어떤 **값**이 발행되고 있는가 |

- Day 1에서 `Twist`·`Pose`에 ①②를 이미 수행했습니다. 오늘은 서비스·액션 타입으로 확장합니다(서비스는 요청·응답 구조라 ③ 대신 호출 결과로 확인)
- 이후 적용 — Day 4 카메라 이미지 · Day 6 초음파 거리 · Day 3 커스텀 인터페이스
...
| 파라미터 (Day 3) | 노드 설정값의 조회·변경 — **서비스 통신을 그대로 사용** | 배경색·주행 속도 |
...
- 구성 = 목표·결과는 **서비스 방식**(요청-응답) · 피드백은 **토픽 방식**(반복 발행) — 액션은 두 방식의 결합입니다

목표 상태의 전이 — `ACCEPTED` → `EXECUTING` → `SUCCEEDED` / `CANCELED` / `ABORTED`

| 상태 | 의미 |
|------|------|
| ACCEPTED | 서버가 목표를 받아들임 — 거부(REJECTED)될 수도 있음 |
| EXECUTING | 수행 중 — 이 구간에서 피드백이 발행됨 |
| SUCCEEDED / CANCELED / ABORTED | 정상 완료 / 클라이언트 요청으로 취소 / 서버 판단으로 중단 |
...
- 작업 터미널에서 `/turtle3/cmd_vel`에 발행하면 **모니터에는 아무것도 출력되지 않습니다** — 토픽 이름이 다르면 별개의 통로입니다
- 모니터 대상을 `/turtle3/cmd_vel`로 바꾸면 그때 메시지가 출력됩니다
...
| 토픽 이름 = 통로의 구분자 | 발행 대상이 다르면 다른 개체가 반응 — Day 3 remapping·Day 8 토픽 계약의 근거 |
| 명령 타입 ↔ 상태 타입 | `Twist`(명령) / `Pose`(상태) 분리 — Day 6 실물도 같은 구도 |
...
**상속과 재정의 — `Node`를 물려받는다는 것**

| 개념 | 이 코드에서 |
|------|------|
| **상속** | `class HelloNode(Node)` — Node의 통신 기능(창구·타이머·로그)을 그대로 물려받음 |
| **재정의(오버라이딩)** | 부모에 있는 메서드를 자식이 **같은 이름으로 다시 정의**해 동작을 바꾸는 것 — `__init__`이 그 사례 |
| **`super()`** | 재정의하면 부모 것이 가려지므로, **부모 동작을 먼저 실행**한 뒤 내 준비를 덧붙임 |

- Day 8 대응 — 공통 판단 노드를 만들어 두고 상황별로 판단 메서드만 재정의하는 구조가 이 원리입니다. 코드를 복사하지 않고 **다른 부분만 바꾸어 사용하는 방식**입니다
...
- 종료 방법 = `Ctrl+C`
- `destroy_node()`의 실행 시점 — 코드상 위치는 spin 바로 아래지만, spin이 무한 대기이므로 **실제 실행 시점 = `Ctrl+C`로 spin이 중단된 직후**입니다. 정리 시점을 코드로 보장하는 관례이며, 한 프로그램에서 노드를 여럿 만들 때는 명시 호출이 필수입니다
...
- 용어의 뜻 — call back = "되부름": 함수를 시스템에 등록해 두면, 시점이 되었을 때 시스템이 그 함수를 호출합니다. 전화 상담에 비유하면 수화기를 들고 계속 기다리는 대신(폴링) **번호를 남겨 두면 순서가 될 때 전화가 되걸려 오는**(콜백) 방식입니다
...
cd ~/ros2_scripts
code .                                     # VS Code를 WSL 연결로 실행 (2.3 참조)
```

VS Code에서 `hello_node.py`를 새로 만들어 6.3의 코드를 입력·저장한 뒤, **터미널에서 실행**합니다(2.1 — 실행은 terminator 쪽 작업 터미널):
...
- **빌드·등록 없이 실행됩니다** — ROS2 환경(`source`)만 되어 있으면 rclpy를 즉시 사용할 수 있습니다
...
| 자율차 대응 | 모터 제어 노드의 원형 | 센서 수신 노드의 원형 (Day 6 초음파) |
...
- 다만 **CLI를 먼저 익히는 것이 기준**입니다 — 실물 로봇은 화면 없이 원격으로 연결하는 경우가 많습니다(Day 6·9)
...
- 이 3단 구조가 최종 프로젝트 자율차의 최소형입니다. Day 3 미로가 이 정지를 회피로 확장하고, Day 8(표지판 반응)·Day 9(장애물 회피)가 같은 뼈대를 이어받습니다
