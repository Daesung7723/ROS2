...
| 도구 | 담당 | 이유 |
|------|------|------|
| **terminator** (분할) | ① 시뮬레이터 · ② **모니터** | 코드와 무관하게 **수업 내내 떠 있어야 하는 것** |
| **VS Code** (WSL 연결) | ③ 스크립트 작성 → **즉시 실행** · ④ 조회 명령 | 편집 흐름 안에서 저장하고 바로 실행하는 자리 |
...
- Day 8부터는 차량(Raspberry Pi 5)에 SSH로 연결합니다. 그때는 VS Code 없이 터미널만으로 작업하므로, 지금 두 방식을 모두 다루어 둡니다
...
- 통합 터미널(`Ctrl + ~`)이 bash가 되므로 **편집과 실행을 한 창에서** 처리할 수 있습니다
...
| 확장 | 설치 위치 | 필수 |
|------|------|:--:|
| **WSL** (Microsoft) | **로컬(Windows)** | 필수 |
| **Python** (Microsoft) | **원격(WSL)** | 필수 |
| **Pylance** | **원격(WSL)** | 필수 (Python 확장과 동반 설치) |
| YAML (Red Hat) | 원격(WSL) | 선택 — Day 3 launch 파일용 |

- 확장 탭을 열면 목록이 **`LOCAL - INSTALLED`**와 **`WSL: UBUNTU-24.04 - INSTALLED`** 두 구획으로 나뉩니다
- Python 확장이 로컬에만 있으면 원격 구획에 다시 설치합니다 — 확장을 검색한 뒤 **`Install in WSL: Ubuntu-24.04`** 버튼

> **자주 하는 실수 —** `import rclpy`에 빨간 밑줄이 표시되는 경우, 원인은 둘 중 하나입니다. ① 창이 WSL에 연결되지 않음(좌하단 표시 확인) ② Python 확장이 원격 쪽에 설치되지 않음. 통합 터미널에서 `python3` 실행은 되더라도 편집기 경고가 남으므로 위 둘을 먼저 확인합니다.
...
VS Code에서 `hello_node.py`를 새로 만들어 6.3의 코드를 입력·저장한 뒤, 통합 터미널(`Ctrl + ~`)에서 실행합니다:
