...
- **Day 4부터는 Raspberry Pi 5에 원격으로 연결**해 작업하며, **Day 8부터는 차량에 장착한 Raspberry Pi 5**가 그 대상입니다. 원격 환경에서는 편집기 없이 터미널만 사용하는 경우가 있으므로, 지금 두 방식을 모두 다루어 둡니다
...
- 진단 수단 — turtle이 움직이지 않을 때 **모니터에 메시지가 출력되는지**로 원인이 둘로 나뉩니다. 출력되면 수신 측 문제, 출력되지 않으면 발행 측 문제입니다. Day 8 실물 차량의 1차 점검도 같은 방식입니다
...
- Day 6 대응 — 공통 판단 노드를 만들어 두고 상황별로 판단 메서드만 재정의하는 구조가 이 원리입니다. 코드를 복사하지 않고 **다른 부분만 바꾸어 사용하는 방식**입니다
