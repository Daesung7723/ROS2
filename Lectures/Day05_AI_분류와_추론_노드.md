...
- 이 부호를 **회전 명령에 실제로 적용하는 코드**는 12장(자습)과 Day 8의 판단 노드에 나타납니다. 오늘 5장의 `color_tracker`는 좌표만 발행하고 회전을 지시하지 않습니다
...
> **Tip —** 어떤 조건에서 틀렸는지 기록해 두십시오. Day 8에서 차량이 오작동할 때 원인을 찾는 근거가 됩니다.
...
- **영상을 확보하지 못한 경우** — Day 4 자료 6장 ⓑ의 저장 영상 재생(14.6)으로 필수 단계를 수행합니다. 재생 중 표지가 화면에 들고 나는 구간이 그대로 전환 시험이 됩니다. 그래도 수행하지 못했으면 **Day 8(10/19) 전까지 카메라를 복구해 필수 단계를 마칩니다** — Day 8은 `/sign_stable`을 전제로 시작합니다
- Day 8 대응 — `/sign_stable`이 **판단 노드의 입력**이 됩니다. 오늘 그 계약을 확정합니다
...
**Day 6 — 초음파 센서·DC모터** (9/28)

초음파 센서와 DC모터를 RPi5에 연결하고, **부품마다 노드를 작성해 동작을 확인**합니다. 초음파 거리는 turtlesim의 움직임으로 확인하고, 모터는 전진·후진·좌회전·우회전을 차례로 시험합니다.

- 준비 — 오늘 만든 `my_car_pkg`를 그대로 사용합니다 · HC-SR04 초음파 센서 모듈 · PiCar-R5 차량 키트(7.4V 배터리 포함) · 브레드보드·점퍼선을 가져옵니다
- 오늘 확정한 토픽 계약 — `/sign_stable`(`String`) · `/target_point`(`Point`)는 **Day 8(10/19) 인식·판단·행동 통합**에서 사용합니다
