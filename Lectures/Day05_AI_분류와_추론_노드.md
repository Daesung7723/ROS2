...
- Day 6부터 차량에 실린 RPi5도 같은 방식으로 다룹니다
...
> **핵심 —** SSH 터미널에서는 그래픽 창이 열리지 않습니다. 오늘 다루는 구간은 모두 터미널로 진행됩니다. 창이 필요한 지점은 2.5에 모아 두었습니다.
...
**②** `F1` → `Remote-SSH: Connect to Host` → `사용자명@192.168.0.__`을 적은 뒤 암호를 입력합니다.

- **좌하단에 `SSH: 192.168.0.__` 표시**가 출력되면 → ③으로 진행
- 첫 연결에서는 RPi5에 VS Code 서버가 자동으로 설치됩니다(외부 인터넷 필요·수 분)
- 연결이 실패하면 → 주소가 맞는지·같은 네트워크에 연결되어 있는지 확인한 뒤 **②를 다시** 실행
- 표시가 없으면 현재 창은 로컬 창입니다 → **②를 다시** 실행
- 서버 설치 표시가 **수 분이 지나도 사라지지 않으면** → 원격 서버 내려받기가 진행되지 않는 상태입니다. ②를 반복하지 말고 **⑤로 전환**하고 교수에게 알림
...
- 설치 버튼이 `Install in SSH: 192.168.0.__`로 나타나는지 확인합니다 → 그러면 원격에 설치되는 상태
- `Install`로 표시되면 로컬에 설치되는 상태입니다 → 원격 창인지 확인하고 **④를 다시** 수행
- 설치되지 않아도 오늘 절차에는 지장이 없습니다(편집 보조 기능) → 2.4로 진행

> **Tip —** 확장이 로컬용과 원격용으로 나뉘는 구조는 Day 2 자료에서 WSL(Windows Subsystem for Linux) 연결로 다룬 것과 같습니다. 연결 대상만 WSL에서 RPi5로 바뀌었습니다.
...
- Day 2·3에서 terminator로 하던 운용을 그대로 옮겼습니다 — 도구가 바뀌어도 방식은 같습니다
- 이 터미널은 RPi5의 셸입니다. `ls`·`colcon build`가 모두 RPi5에서 실행됩니다
- **2.3 ⑤로 진행한 경우** — VS Code 대신 PC의 터미널에서 `ssh`로 **세 번 연결**해 같은 터미널 3개를 구성합니다

**터미널 ① — 카메라 노드 실행**

1.2에서 점검용으로 실행한 카메라 노드를 `Ctrl+C`로 종료하고, 터미널 ①에서 다시 실행합니다. 오늘 작성하는 노드는 모두 이 카메라 노드가 발행하는 영상을 구독하므로 **수업이 끝날 때까지 이 터미널을 유지**합니다.
...
- `Package 'camera_ros' not found`가 나오면 → Day 4 자료 5.1 **⑤-1**(환경 등록)을 수행하고 **다시** 실행
- `no cameras available`이 나오면 → Day 4 자료 5.1 **⑤-2**(실행 경로 확인)부터 수행
- 해상도를 지정하는 이유 — 지정하지 않으면 기본값(800×600)으로 시작합니다. 5장의 면적 기준값은 **640×480 해상도**를 전제로 정했으므로 크기를 맞추어 둡니다(3.4에서 실제 값을 확인)
...
- SSH에서 실행하면 **표시 장치에 연결할 수 없다**는 오류가 출력됩니다. 고장 때문이 아니라 화면이 없는 환경에서 창을 열려고 했기 때문입니다
- 창이 필요하면 → RDP로 연결해 그 화면에서 실행 → 그래도 어려우면 RPi5에 모니터를 연결

### 2.6 토픽 원격 관찰

RPi5에서 실행 중인 노드의 토픽을 **PC에서 직접 관찰**할 수도 있습니다. 이 방식은 아래 세 조건을 모두 충족할 때만 동작합니다.

| 전제 | 확인 방법 |
|------|------|
| ① PC와 RPi5가 같은 공유기(같은 서브넷)에 연결 | 두 기기 주소의 앞 세 부분(예: `192.168.0`)이 같은지 비교 — RPi5 `hostname -I` · PC `ipconfig` |
| ② 같은 `ROS_DOMAIN_ID` | 두 기기에서 `echo $ROS_DOMAIN_ID` |
| ③ PC가 WSL2일 때의 네트워크 연결 방식 | 기본 방식인 NAT(Network Address Translation)에서는 다른 기기의 토픽이 보이지 않을 수 있음 — 교수 확인 |
...
**카메라가 구동되지 않은 경우(Day 4 자료 6장 ⓐ)** — 정상인 다른 RPi5의 영상을 **각자의 RPi5에서** 구독합니다. 두 기기가 모두 RPi5이므로 위 표의 전제 ③은 해당하지 않습니다.
...
- 번호는 **영상을 제공하는 RPi5에 맞춥니다** — 개인 번호는 Day 1에서 정한 값이지만, 이 경우에만 일시적으로 같게 둡니다

> **핵심 —** DDS(Data Distribution Service)는 같은 네트워크 안에서 멀티캐스트로 다른 기기를 자동으로 탐색합니다. 공유기를 넘어서 찾는 것은 기본 동작이 아닙니다. 또 공유기의 무선 클라이언트 격리나 PC 방화벽이 켜져 있으면 토픽이 보이지 않습니다.
...
**학습 목표** — 색으로 대상을 찾는 원리와 HSV(Hue·Saturation·Value) 색공간을 사용하는 이유를 설명할 수 있다.

### 4.1 픽셀에서 좌표로

영상은 화소값의 배열일 뿐입니다. "빨간 공이 어디 있는가"를 알려면 **화소값에서 위치를 추출하는 절차**가 필요합니다.
...
- HSV에서는 **H(색상)가 변하지 않음** — "빨강"이라는 판단이 유지됨
...
> **자주 하는 실수 —** 빨강은 H 범위가 양끝 두 구간으로 나뉩니다. 한 범위만 사용하면 절반을 놓치므로 두 범위를 만들어 합쳐야 합니다. 5장의 코드에서는 `h_min`을 `h_max`보다 크게 지정하면 두 범위를 합쳐 검출합니다(5.4 「색 값 정하기 ③」).
...
- 커널 크기가 크면 잡음이 강하게 제거되지만 **작은 대상도 함께 소거됨**

### 4.4 무게중심 계산

흰 영역의 중심은 **모멘트**로 구하는 것이 표준입니다.
...
- 면적(`m00`)은 **거리 추정에도 사용됨** — 가까울수록 커짐(5.3 ⑨에서 검출 판정에 사용)
...
| 2 | 그늘로 들어가면 BGR과 HSV 중 어느 값이 더 크게 변하는가? | 4.2 — HSV를 사용하는 이유 |
| 3 | 마스크에 작은 흰 점이 많다. 어떤 연산을 사용하는가? | 4.3 — 열림 연산 |
| 4 | 대상이 화면 **오른쪽**에 있다. `angular.z`는 양수인가 음수인가? | Day 4 — **부호 반전** |
| 5 | `m00`이 0일 때 무엇을 확인하지 않으면 오류가 발생하는가? | 4.4 — 나눗셈 전 확인 |

- 4번이 오늘 가장 자주 틀리는 지점입니다 — **영상의 오른쪽(+)과 로봇의 반시계(+)가 반대**
- 이 부호를 **회전 명령에 실제로 적용하는 코드**는 12장(자습)과 Day 8의 판단 노드에 있습니다. 오늘 5장의 `color_tracker`는 좌표만 발행하고 회전을 지시하지 않습니다

---

## 5. 색상 검출 노드

**학습 목표** — 영상 토픽(`Image`)을 구독해 OpenCV로 처리하는 노드를 작성하고, 검출 결과를 좌표로 발행할 수 있다.
...
**의존성 선언** — 패키지가 이 라이브러리를 사용한다고 `package.xml`에도 적습니다(Day 3 자료와 같은 절차 · **import가 늘면 선언도 늘립니다**).
...
- 저장한 뒤 → 5.2로 진행
- 이미 있는 줄은 다시 추가하지 않습니다(중복 선언 시 빌드 경고)
- 선언하지 않아도 이 기기에서는 빌드되지만, **다른 기기에서 이 패키지를 내려받아 빌드할 때** 의존 패키지가 자동으로 설치되지 않습니다
...
| 파라미터 | `h_min`·`h_max`·`s_min`·`v_min` — 색 범위를 코드 수정 없이 조정 |
| 동작 | 영상 수신 → HSV 변환 → 마스킹 → 열림 → 무게중심 → 발행 |

- 좌표 전달에 `Point`를 사용하는 이유 — x·y·z 세 실수를 담는 **표준 타입**이므로 별도 정의가 불필요
- 색 범위를 파라미터로 분리하는 이유 — **조명이 바뀌면 값을 다시 맞추어야 함**
...
| ⑩ | 화면 가운데의 HSV 값을 1초마다 출력 — 물체의 색 값을 측정하는 용도(5.4) |
...
| `cv2.moments(mask, binaryImage=True)` | 키워드 인자 — `True`면 흰 픽셀을 1로 취급 → `m00` = 픽셀 개수 |
...
| `if h_min <= h_max:` … `else:` | 범위가 0을 넘어가는지로 분기 — 빨강만 `else` 블록 실행 |
...
- 세 표의 항목은 Day 5·6·8·9의 모든 노드 코드에 **같은 형태로 반복**됩니다 — **이후 Day의 자료에도 같은 표를 다시 싣습니다.** 지금 전부 암기할 필요는 없으며, 코드를 읽을 때마다 표를 참조해 확인합니다
...
# 터미널 ① — 2.4에서 실행해 둔 카메라 노드를 그대로 둡니다(종료되었으면 2.4의 명령으로 다시 실행)
# 터미널 ②
...
기본값은 **초록** 범위입니다. 초록색 물체가 있으면 먼저 관찰합니다. 다른 색 물체를 사용하려면 아래 「내 물체의 색 값 정하기」로 값을 정한 뒤 관찰합니다.
...
- **영상을 확보하지 못한 상태**면 → Day 4 자료 6장의 ⓐ·ⓑ로 `/camera/image_raw`가 발행되게 한 뒤 이 절로 돌아옴
...
| 조건 | 이유 |
|------|------|
| **단색** | 여러 색이 섞이면 한 범위로 검출되지 않음 |
| **무광** | 반사광 부분은 흰색이 되어 마스크에서 제외됨 |
| **배경과 다른 색** | 같은 색 배경은 함께 잡혀 구분 불가 |
| 손바닥 크기 | 너무 작으면 면적 500 미만 = 미검출 |

- 모양은 관계없습니다 — 색만으로 찾습니다(속이 빈 고리 모양은 중심점이 빈 곳에 표시됨)
...
| `s_min` | S의 절반 (계산값이 50보다 작으면 50) | 90 |
| `v_min` | V의 절반 (40보다 작으면 40) | 70 |

- V를 절반으로 정하는 이유 — **그늘에서는 V가 절반으로 낮아집니다**(4.2)
- 출력값이 매번 크게 달라지면 → 물체가 가운데에서 벗어났거나 반사광이 비친 상태 → 물체를 옮기고 **2부터 다시**

**내 물체의 색 값 정하기 ③ — 빨강(H가 0 근처)**

H 값은 179 다음에 0으로 이어집니다. 계산 결과가 0보다 작거나 179보다 크면 **180을 더하거나 빼서** 넣습니다.
...
- `h_min`이 `h_max`보다 크면 코드 ⑥에서 **두 구간을 합쳐** 검출합니다(5.3)

**내 물체의 색 값 정하기 ④ — 시작 값 참고**

색 값을 측정하기 전에 먼저 넣어 볼 값입니다.
...
**색 범위 조정** — 정한 값을 실행 중인 노드에 넣습니다(코드 수정 없음). 대상을 비추어도 x = -1.0만 나오면 값을 다시 조정합니다:
...
`m00 > 500`의 500(면적 기준)은 `/target_point`의 `z`(면적)를 `echo`로 보면서 대상이 있을 때와 없을 때의 값을 비교해 그 사이로 정합니다.

> **자주 하는 실수 —** ① 조명이 바뀌면 값이 맞지 않습니다(창가와 실내 조명 아래에서 측정값이 다릅니다) ② `m00 > 500`의 500은 해상도에 따라 달라집니다. 3.4에서 확인한 해상도가 640×480보다 높으면 같은 대상의 면적도 커집니다. 검출이 지나치게 민감하면 500을 올립니다.
...
- 이 도구는 **창이 필요하므로 SSH 터미널에서는 열리지 않습니다**(2.5) → **RDP로 연결해 원격 데스크톱 화면에서 실행**하거나, 아래처럼 한 장을 파일로 저장해 VS Code에서 엽니다
...
- 추가 위치 — 첫 줄은 `self.pub = ...` 다음 줄에 **같은 들여쓰기**로, 둘째 줄은 `self.pub.publish(point)` 다음 줄에 추가합니다(들여쓰기가 어긋나면 다른 메서드에 속하게 됨)
...
- 위 세 설정은 **Advanced**를 열어야 표시됩니다 — 기본 화면에는 **Train Model** 버튼만 있습니다. 오늘은 값을 바꾸지 않으므로 열지 않아도 됩니다
- **1~3분** 소요 — 이 대기 시간에 8.2를 진행합니다
- 학습이 끝날 때까지 **Teachable Machine 탭을 화면에 띄운 상태로 유지**해야 함 — 다른 탭으로 이동하면 브라우저가 학습을 늦추거나 멈춤 · 창을 닫으면 처음부터 다시 학습
- 진행 막대가 멈춘 채 5분이 지나면 → 탭을 새로 고치지 말고 교수에게 알림(새로 고치면 촬영분이 삭제됩니다)

### 8.2 왜 1~3분 만에 끝나는가

이미지 인식 모델을 처음부터 학습하려면 수백만 장의 사진과 긴 계산 시간이 필요합니다. 이번 실습에서는 클래스당 100장 내외의 사진으로 학습을 몇 분 만에 마쳤습니다. **전이 학습(Transfer Learning)** 방식이기 때문입니다.

| 구간 | 역할 | 학습 여부 |
|------|------|------|
| **특징 추출부** | 윤곽·모서리·질감 등 **일반적인 시각 특징** 인식 | ❌ 고정 — 사전 학습 그대로 |
| **분류부** | 그 특징을 **학습자가 정한 클래스에 대응** | ✅ 직접 촬영한 사진으로만 재학습 |

- Teachable Machine은 대규모 데이터로 미리 학습된 모델(MobileNet 계열)을 내장하고, 마지막 분류부만 직접 촬영한 사진으로 다시 학습합니다
- 따라서 **적은 데이터·짧은 시간**으로 실용 수준에 도달합니다
- 한계 역시 이 구조에 있습니다 — 특징 추출부가 일반 사진을 기준으로 학습되었으므로 성격이 크게 다른 영상(열화상 등)에서는 정확도가 낮아집니다
...
| 시도 | 예상 결과 |
|------|------|
| 표지를 **아주 멀리** 둠 | 신뢰도 하락 · 오분류 |
| **비스듬히** 크게 기울임 | 오분류 |
| **손으로 절반을 가림** | 오분류 |
| **찍지 않은 배경**에서 보여줌 | 오분류 |
| **다른 조명**(창가 등) | 신뢰도 하락 |

> **이것이 오늘의 학습 내용입니다 —** 오분류는 **모델의 결함이 아니라 데이터의 공백**입니다. 학습 데이터에 없던 조건이 입력되었기 때문입니다. **코드를 고쳐서 해결할 수 없습니다.**

### 8.5 데이터 보강과 재학습

오분류가 발생한 조건을 **추가로 촬영**해 다시 학습합니다.
...
- 실무의 AI 개발도 대부분 이 순환을 따릅니다. "모델을 바꾸는 것"보다 "**데이터를 채우는 것**"이 효과가 큽니다

> **Tip —** 모델이 어떤 조건에서 오분류했는지 기록해 둡니다. Day 8에서 차량이 오작동할 때 원인을 찾는 근거가 됩니다.
...
**학습 목표** — 모델을 불러와 카메라 영상에 적용하는 노드를 작성하고, 판정 결과와 신뢰도를 토픽으로 발행할 수 있다.
...
- 위 표의 `python3-opencv`·`cv_bridge`는 **5.1을 수행한 경우 이미 설치된 상태**입니다. 5장을 건너뛰었다면 아래 명령을 먼저 실행합니다
...
- `externally-managed-environment` 오류가 나오면 → `--break-system-packages`를 빠뜨렸기 때문입니다. **①을 다시** 실행
- `Retrying`·`Read timed out`·`Temporary failure in name resolution`이 나오면 → 외부망 연결 문제입니다. ①을 반복하지 말고 **③ 오프라인 설치**로 진행
...
교수가 미리 내려받아 둔 설치 파일(`.whl`)을 USB 메모리나 9.3 ②와 같은 방식(VS Code 탐색기로 끌어다 놓기)으로 전달받습니다. 그 파일이 있는 폴더에서 아래 명령을 실행합니다.
...
- 그래도 실패하면 → 교수에게 알림. 이 경우 10~11장의 동작은 옆자리 화면으로 확인하고, 자신의 코드 작성·`setup.py` 등록·빌드까지는 그대로 수행합니다

> **형식과 실행기는 다릅니다 —** 모델 파일 형식은 `.tflite`로 동일하지만, 그 파일을 읽어 구동하는 **실행기(런타임)의 이름**이 바뀌었습니다. 구글은 TensorFlow Lite 런타임을 **LiteRT**로 개칭했습니다. 파이썬 패키지 이름도 `tflite-runtime`에서 `ai-edge-litert`로 변경되었습니다. 인터넷의 예제 다수가 아직 `tflite-runtime`을 사용합니다. **Ubuntu 24.04의 Python 3.12에는 `tflite-runtime`이 설치되지 않으므로**(지원 범위가 3.11까지) `ai-edge-litert`를 사용합니다. 코드에서 달라지는 것은 **import 줄과 클래스 이름 두 곳뿐**입니다.
...
> **매 프레임 추론하지 않습니다 —** 카메라 노드는 초당 30장을 발행합니다. RPi5 CPU로 30번 추론하면 다른 처리가 지연됩니다. **초당 5회 정도**면 표지판 판정에 충분합니다.
...
class SignClassifier(Node):
    def __init__(self):
        ...
        self.declare_parameter('period', 0.2)                     # ② 처리 주기(초)
...
| ⑧ | 정규화 — Teachable Machine 모델이 요구하는 입력 범위는 **−1~1** |
| ⑨ | 최고 점수 선택 → **하한 미만이면 `none`** |

> **자주 하는 실수 —** ⑧의 정규화 범위를 −1\~1이 아니라 0\~1로 두거나, ⑦ 다음의 BGR→RGB 변환을 빠뜨리면 **오류 없이 실행되면서 판정만 특정 클래스로 치우칩니다.** 실행된다고 해서 전처리가 정확한 것은 아닙니다 — 신뢰도가 계속 비정상이면 ⑦·⑧을 먼저 확인합니다.
...
**코드 읽기 — Python 문법 ⑤ 파일 읽기와 문자열** — `labels.txt`에서 라벨 이름만 남기는 한 줄에 쓰인 문법입니다.
...
| 코드 | 뜻 |
|------|------|
| `if self.latest is None:` | `is None` = 값이 아직 없음을 확인하는 표준 형태 — `== None`은 사용하지 않음 |
| `A if 조건 else B` | 조건부 표현식 — 한 줄로 값을 선택. 5.3 문법 ③의 `if`·`else` 블록과 뜻은 같으나 결과가 값이라는 점이 다름 |
| `String(data=name)` | 메시지 객체를 만들며 필드 값을 함께 지정 — 키워드 인자 |
| `f'... {self.labels}'` | f-문자열 — 따옴표 앞에 `f`, 중괄호 안 변수의 값이 그 자리에 삽입됨 |
| `try:` … `except:` … `finally:` | Ctrl+C로 종료해도 `finally`의 정리는 반드시 실행됨 |

- **Day 6·8·9의 코드에도 위 네 표를 같은 형태로 다시 싣습니다** — 지금 전부 암기할 필요는 없습니다
...
터미널 ②에 `sign_classifier ready — labels: [...]`가 표시되고 `/sign`에 판정이 출력되면 → 아래 관찰로 진행합니다. 오류는 종류에 따라 돌아갈 단계가 다릅니다.
...
| 치움 | `/sign` = `none` |
| 표지를 절반가량 가림 | 신뢰도 하락 → 하한 미만이면 `none` |
...
- **Day 9 대응** — 실물에서는 이 노드와 모터 제어가 동시에 실행되어야 하므로 CPU 여유를 남겨야 함

### 10.5 실행 중 파라미터 조정

노드를 재시작하지 않고 파라미터를 바꾸어 봅니다.
...
| 주기 변경 | `period` → **0.05** | **`ros2 topic hz /sign` 값에 변화 없음** — 반영되지 않음 |

**`threshold`는 즉시 반영되지만 `period`는 변하지 않습니다.** 차이는 코드가 파라미터를 읽는 시점에 있습니다.
...
- **자유 조작** — `threshold`를 여러 값으로 조정하며 11장에서 사용할 자신의 기준값을 미리 확인해 둡니다
...
### 11.1 과제 — 변동하는 판정의 안정화

`/sign`을 계속 관찰하면 문제가 드러납니다.

| 증상 | 원인 |
|------|------|
| `stop`과 `none`이 빠르게 번갈아 출력됨 | 경계 조건일 때 신뢰도가 하한 부근에서 변동함 |
| 손이 지나가는 순간 오판정 | 순간적인 잘못된 입력 |
| 표지를 치운 뒤에도 잠시 유지 | 마지막 값이 남음 |

**차량이 이 신호로 정지·주행을 결정한다면** 이 변동이 그대로 동작에 나타납니다. 안정화가 필요합니다.
...
이 실습에서는 **연속 N회 일치**를 사용합니다 — 가장 단순하고 효과가 분명합니다.
...
- N이 작으면 빠르지만 변동이 큼 — **`period`와 함께 조정**(N=3, period=0.2 → 0.6초 후 확정)
...
| `/sign_stable` | Day 8의 판단 노드가 구독할 토픽 |
...
- `append`·`pop`·`len`·`set`은 ROS2 고유 문법이 아니라 **Python 기본 자료형의 기능**입니다 — Day 8 판단 노드에서 같은 형태로 다시 사용합니다

**추가 위치** — 위 두 부분을 10.3의 `sign_classifier.py`에 넣습니다.
...
- `if len(self.history) == n and ...` 안쪽의 `if`는 **한 단 더** 들여씁니다(들여쓰기 3단). 한 칸만 어긋나도 `IndentationError`가 발생하거나 다른 메서드에 속하게 됩니다
...
- `/sign`은 출력되지만 `/sign_stable`이 없으면 → `__init__`의 `stable_pub` 선언과 `infer` 말미의 발행 줄이 **둘 다** 들어갔는지 확인하고 **빌드부터 다시** 실행
- 확정이 전혀 바뀌지 않으면 → `stable_n`을 낮추어(`ros2 param set /sign_classifier stable_n 2`) 확인

### 11.4 단계별 과제

| 단계 | 과제 |
|:--:|------|
| **필수** | `/sign_stable`을 발행하고, 표지를 들었다 놓았을 때 **값이 번갈아 바뀌지 않고 한 번에 전환**되는지 확인 |
| **도달** | `stable_n`·`threshold`·`period`를 조정해(10.5) **반응 속도와 안정성의 균형**을 찾고 `param dump`로 저장 |
| **도전** | ⓐ 신뢰도 평균 방식으로 바꾸어 비교 ⓑ 판정이 바뀐 시각을 로그에 남겨 **반응 지연을 측정** ⓒ 5장의 `color_tracker`와 함께 실행해 **CPU 여유** 확인 |

- **필수 단계는 전원 완료** — 안정화된 토픽 하나가 출력되면 성공
- **영상을 확보하지 못한 경우** — Day 4 자료 6장 ⓑ의 저장 영상 재생(14.6)으로 필수 단계를 수행합니다. 재생 중 표지가 화면에 나타났다 사라지는 구간이 그대로 전환 시험이 됩니다. 그래도 수행하지 못했으면 **Day 8(10/19) 전까지 카메라를 복구해 필수 단계를 마칩니다** — Day 8은 `/sign_stable`을 전제로 시작합니다
- Day 8 대응 — `/sign_stable`이 **판단 노드의 입력**이 됩니다. 오늘 이 토픽 규격(이름·메시지 타입)을 확정합니다
...
- 자습 과제이므로 제출하지 않습니다 — **1단계까지만 수행해도** Day 8의 판단 노드를 이해하는 데 충분합니다
...
| 항목 | 내용 |
|------|------|
| 규칙 ↔ 학습 | 색 검출은 사람이 규칙을 작성 / 분류는 **데이터로 학습**. "빨간 것"과 "정지 표지판"의 차이 |
| **핵심 원칙** | **결과는 코드가 아니라 데이터로 결정됨** — 오분류는 모델 결함이 아니라 **데이터의 공백** |
| 클래스 설계 | 구분 대상 + **반드시 `none`(배경) 클래스** — 없으면 빈 화면도 특정 클래스로 분류됨 |
| 촬영 조건 | 거리·각도·조명·배경을 **의도적으로 나누어** 촬영. 한 자리 연속 촬영은 무의미 |
| 학습 순환 | 학습 → 검증 → **오분류 관찰 → 데이터 보강 → 재학습** (최소 1회) |
| 배포 | **TFLite** = 소형 기기 추론 전용 **형식**. `model_unquant.tflite` + `labels.txt`(순서 = 출력 순서) |
| 실행기 | **LiteRT**(`ai-edge-litert`) — 형식과 실행기는 별개. 코드에서는 `Interpreter` 하나만 사용 |
| 패키지 등록 | `setup.py`의 `data_files` — **코드 외 파일도 선언해야 빌드 결과물에 포함됨** |
| 추론 노드 | 모델 적재는 **한 번만** · 구독 콜백은 최신 프레임 보관만 · **타이머로 주기 추론**(카메라 속도와 분리) |
| 안정화 | **연속 N회 일치**로 확정 — 변동하는 판정을 그대로 차량에 전달하지 않음 |
...
**Day 6 — 초음파 센서·DC(직류) 모터** (9/28)

초음파 센서와 DC모터를 RPi5에 연결하고 **부품마다 노드를 작성해 동작을 확인**합니다. 초음파 거리 측정값을 turtlesim의 움직임으로 점검합니다. 모터의 전진·후진·좌회전·우회전을 차례로 시험합니다.

- 준비 — 오늘 만든 `my_car_pkg`를 그대로 사용합니다. HC-SR04 초음파 센서 모듈 · PiCar-R5 차량 키트(7.4V 배터리 포함) · 브레드보드·점퍼선을 가져옵니다
- 오늘 확정한 토픽 규격(이름·메시지 타입) — `/sign_stable`(`String`) · `/target_point`(`Point`)를 **Day 8(10/19) 인식·판단·행동 통합**에서 사용합니다
