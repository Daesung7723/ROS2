...
Day 4·5에서는 RPi5의 **카메라 영상과 AI 분류**를 다루었습니다. 오늘은 RPi5의 **GPIO**(General-Purpose Input/Output — 범용 입출력) **핀에 실물 부품을 연결**합니다 — 코드로 부품을 직접 제어하는 첫 실습입니다.
...
오늘의 목적은 **부품마다 노드를 만들어 동작을 확인하는 것**입니다. 부품을 결합해 센서 값에 따라 자율 주행하는 차량은 Day 8(turtlesim 통합)·Day 9(실물 통합)에서 완성됩니다.
...
> **오늘의 순서 원칙**
>
> **결선이 끝나지 않은 상태에서 코드로 넘어가지 않습니다.** 부품마다 ⓐ 결선 → ⓑ 육안 점검 → ⓒ ROS2 없이 단품 스크립트로 통전 확인 → ⓓ ROS2 노드 작성의 순서를 지킵니다. 코드 오류와 배선 오류가 섞이면 원인을 찾기 어렵습니다.

오늘은 **오전에 초음파 센서, 오후에 모터**를 다룹니다. 부품마다 작업을 완결하며 진행합니다.
...
| **PiCar-R5 차량 키트** | 1 | 조립 완료 여부 · 자동차 제어보드 · DC모터 4 · 바퀴 4 |
| **7.4V 배터리** (키트 구성품) | 1 | **모터 전원** · 충전 상태 |
| 브레드보드·점퍼선 | 필요 수량 | 수-암·수-수 혼용 |
| **저항 1kΩ · 2kΩ** | 각 1 | Echo 분압용(3.2) |

> **오늘의 공통 원칙 — 전원을 끄고 배선합니다**
>
> **전원이 연결된 상태에서 선을 연결하거나 분리하면 부품·보드가 손상될 수 있습니다.** 모든 배선 작업은 RPi5 전원과 배터리를 분리한 상태에서 수행합니다. 육안 점검을 마친 뒤에 전원을 연결합니다.
...
> **단위를 계약에 명기합니다**
>
> ROS 표준 단위는 m입니다(Day 1 자료 2.3). 이 과정은 **cm를 사용**합니다 — 0~400의 정수 범위로 읽기 쉽고 2025년 검증 코드와 단위가 같기 때문입니다. 표준과 다른 단위를 사용할 때의 규칙은 하나 — **메시지 정의와 계약에 단위를 명기**해 모든 노드가 같은 해석을 공유하는 것입니다(Day 3 자료 7.2).

### 4.2 DistMsg 정의 — RPi5에 인터페이스 패키지 만들기

> **선행 확인 — Day 3 자료 7장 커스텀 인터페이스**
>
> Day 3 자료 7장에서 **PC**(WSL)에 `my_msgs`를 만들었습니다. 빌드 결과는 만든 기기에 종속되므로(Day 5 자료 3.1) **RPi5에는 새로 만듭니다.** 절차는 Day 3 자료 7.2~7.4와 같습니다 — 그때의 필기와 해당 자료를 참조합니다.
...
- `my_msgs`가 이미 있었다면(② 시작) — ③의 `rosidl_generate_interfaces` 줄에 `"msg/DistMsg.msg"`를 **추가**합니다(메시지 파일을 새로 만들 때마다 이 줄에 파일명을 함께 적습니다 — Day 3 자료 7.3)
...
- 2025년 원 예제에서는 `rasp5_msg`라는 **별도 인터페이스 패키지**를 만들었습니다 — 이 과정은 `my_msgs`라는 이름을 사용합니다. 두 방식 모두 구조는 동일(인터페이스 전용 ament_cmake 패키지)
...
> - GPIO 접근 권한 오류가 나면 — 재로그인한 뒤 다시 시도합니다. 그래도 지속되면 `sudo`로 실행해 원인을 구분합니다
...
| `msg.dist = self.sensor.distance * 100.0` | 점 연결로 읽은 값에 곱셈 후 필드에 대입 — `100.0`이 실수이므로 결과도 실수 |
| `f'{sensor.distance * 100:.1f} cm'` (3.4) | f-문자열 **형식 지정** — 소수 첫째 자리까지 |
| `while True:` … `sleep(0.5)` (3.4) | 무한 반복 + 지연 — ROS2 노드의 타이머가 대신하는 구조 |

- 표 ①·②의 형태는 **4.5·6.4·7.2의 노드와 이후 Day의 코드에도 다시 나타납니다** — 지금 전부 암기할 필요는 없으며, 코드를 읽을 때마다 표를 참조하며 확인합니다
...
- 교체할 노드의 토픽 이름이 다르면 코드를 고치지 않고 **실행할 때 이름만 바꾸어** 연결합니다(remapping — 8.1)
...
| `cmd = Twist()` | 메시지 객체 생성 — 필드는 모두 기본값 0(4.3 코드 읽기 ②의 `DistMsg()`와 같은 형태) |
...
- turtle이 움직이지 않으면 → 터미널 ③에 명령이 출력되는지 확인 → 출력되지 않으면 `ros2 topic echo /us_dist`로 거리 값을 확인(4.4) → 거리 값이 출력되면 터미널 ②의 오류를 확인하고 **빌드부터 다시** 실행
...
- 손을 댄 뒤 turtle이 멈출 때까지 **약간의 지연**이 있습니다 — 측정 주기(0.1초)와 전달·판단에 필요한 시간이 차례로 누적된 결과입니다. 실물 차량은 이 지연 동안에도 계속 전진하므로 **기준 거리에 여유가 필요합니다**(Day 9에서 계산)
...
| 2 | 정지 기준을 바꾸려면 어느 노드를 수정하는가? 실행 중이라면? | 판단 노드 · `param set` |
...
`/us_dist`의 요동(4.4)을 완화하는 표준 수단은 **최근 N개의 평균**을 발행하는 이동 평균입니다.
...
- 원본 `/us_dist`와 필터 `/us_dist_filtered`를 함께 발행 — rqt_plot에 두 토픽을 겹쳐 효과를 시각적으로 비교
...
- 표준 타입이 있으면 그것을 사용하는 것이 실무 원칙 — 커스텀은 **표준으로 표현되지 않을 때**(Day 3 자료 7.1)
...
- 진단 순서 — **배선(6.2) → 전원(이 표) → 코드** 순으로 확인. 코드는 마지막에 검토합니다

### 10.5 거리에 비례하는 속도 — 비례 제어

4.5 `dist_turtle`의 속도는 두 값뿐입니다 — 기준 거리 이상이면 `speed`, 미만이면 0. 이와 같이 명령이 켬·끔 두 값뿐인 방식을 **온오프 제어**(on-off control)라고 합니다.

- turtle은 기준 거리까지 최고 속도로 전진하다가 한 번에 정지합니다
- turtle은 즉시 멈추지만, 실물 차량은 관성 때문에 정지 명령 뒤에도 조금 더 전진합니다
- 개선 방향 = **가까워질수록 속도를 줄여** 기준 거리에서 0이 되게 하는 것
...
| ② | 세 값을 지역 변수로 먼저 읽어 식을 짧게 작성 — 매 호출마다 읽으므로 `param set`으로 바꾼 값이 다음 호출부터 반영 |
...
**코드 읽기 — Python 문법 ⑧ 지역 변수와 비율 계산** — 7.2 ⑥⑦의 클램프·괄호 계산을 다시 다룹니다.
...
- Topic 입력란에 `/turtle1/cmd_vel/linear/x`를 적고 추가(Day 2 자료 7.6 실습 2와 같은 방법)
- 그래프에는 **속도만** 표시 — 거리(0\~200cm)를 함께 표시하면 세로축이 거리 범위로 맞추어져 속도(0\~1.0)의 변화가 거의 드러나지 않습니다
...
| 실험 | 방법 | 관찰 |
|------|------|------|
| 천천히 접근 | 손을 80cm → 20cm로 이동 | 속도가 1.0에서 0까지 **비스듬히** 감소 · turtle이 서서히 정지 |
...
- 두 노드의 결과가 한 그래프 창에 차례로 그려지므로 모양의 차이를 비교할 수 있습니다
