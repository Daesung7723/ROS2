...
오늘 완성할 것 — **멀티 노드 구성으로 미로를 자율주행하는 turtlesim** (Day 8 판단 노드·Day 9 실물 회피 주행의 예행)
...
- 패키지 = **공유·재사용의 단위** — ROS 생태계에서 코드가 배포되는 형태입니다
...
> **자주 하는 실수 —** 빌드 직후 같은 터미널에서 `ros2 run`을 실행하면 새로 등록한 노드를 찾지 못합니다. `.bashrc` 등록은 새 터미널에만 적용되므로, 빌드한 터미널에서는 `source install/local_setup.bash`를 직접 실행해야 합니다.
...
> **Tip —** Python 패키지는 `colcon build --symlink-install`로 빌드하면 코드 수정 시 재빌드 없이 즉시 반영됩니다(install에 복사 대신 원본 연결을 배치하는 옵션). 오늘처럼 코드를 자주 수정하는 날에 유용합니다.
...
- 자율차 대응 — 주행 속도·회피 거리·조향 각도를 코드 밖으로 분리하면 **주행 중 튜닝**이 가능합니다(Day 6·9 실물 조정에서 필수)
...
- 확인 — **코드는 동일하지만 궤적이 달라집니다.** 세 번의 실험에 재빌드가 없었습니다
...
ros2 param set /circle_driver angular_speed 2.0   # 주행 중 변경 — 궤적이 즉시 좁아짐
...
**설정 저장과 재사용** — `param set`으로 조정한 값은 **노드를 종료하면 사라집니다.** 확정한 설정을 다음 실행에 재사용하려면 파일로 저장합니다.
...
| **`--params-file`** | **yaml 파일** | **영구 — 재사용·공유 가능** |

- 실무 흐름 — `param set`으로 **튜닝** → `param dump`로 **저장** → `--params-file`로 **재현**
- Day 6·9 대응 — 실물 차량의 회피 거리·모터 보정값을 이 방식으로 차량별 파일로 관리합니다
...
- (3) 삼각형 — 회전 1초에 120°가 되려면 2.09rad/s입니다. Day 2에서는 **코드를 수정해** 바꿨으나 지금은 **같은 노드에 다른 값을 주입**해 바꿉니다
...
- 지금 빌드가 성공하는 이유 = 내 컴퓨터에 `rcl_interfaces`가 ROS2 본체와 함께 이미 설치되어 있기 때문입니다
...
| 2 | 그대로 노드 실행 | **정상 동작** — 내 환경에는 설치되어 있으므로 |
...
| 표지판 인식 결과 (Day 8) | 분류 이름 + 신뢰도 | 문자열·실수를 따로 발행 |
| 초음파 거리 (Day 6) | 거리 값 | `Float32`로 가능하나 의미가 드러나지 않음 |
...
> - 인터페이스를 수정한 뒤에는 **`my_msgs`부터 빌드**해야 합니다. 사용하는 쪽만 빌드하면 옛 정의가 남습니다
...
| Day | 용도 |
|:--:|------|
| **6** | **RPi5 초음파 거리 퍼블리셔** — 거리 값을 담는 `.msg`를 정의해 실물 센서 노드에 사용 |
| 8 | 인식 노드 → 판단 노드로 보내는 **분류 이름 + 신뢰도** |
...
- Day 8(시뮬레이션 통합)·최종 프로젝트에서 노드 4~5개를 이 방식으로 운영합니다
...
- Day 8·Day 9 대응 — 카메라 해상도·회피 거리 같은 설정이 늘어나면 launch 파일이 **차량 전체의 설정 파일** 역할을 합니다
...
- 주행 방식 — **랜덤 주행**(전진 + 무작위 조향)입니다. 경로를 미리 계산하지 않으며, **2단계에서도 이 조건은 유지**합니다
- 회피 — 외곽 경계·가상 벽을 감지하면 **후진·회전으로 회피하고 계속 주행**합니다(Day 2 도전의 경계 정지에서 확장)
...
| 단계 | 과제 | 완료 기준 |
|:--:|------|------|
| **1단계** | 랜덤 주행 + 회피로 **목표 구역 도달·완주** | 목표 구역 진입 시 정지 + 도달 시간 로그 |
| **2단계** | 같은 미로·같은 조건에서 **도달 시간을 단축하는 랜덤 주행 알고리즘** 구현 | **3회 평균**이 1단계보다 단축 |
...
- 인식 → 판단 → 행동의 3단 결합입니다 — **Day 8 판단 노드·Day 9 실물 회피와 같은 뼈대**이며, 센서가 Pose에서 카메라·초음파로 바뀔 뿐입니다
...
**최소 완성** — `maze_driver.py`: RUN/BACK/TURN 상태 기계 + 벽·경계 감지. 패키지 등록·`ros2 run`으로 실행되고, **충돌·이탈 없이 주행이 지속**되면 회피 알고리즘이 성립한 것입니다.

1. **1단계 — 목표 도달·완주** — 목표 구역 진입 시 정지 + **도달 시간 로그**(`goal! 42.3s`) + **launch로 pose_printer와 일괄 실행**
2. **2단계 — 도달 시간 단축** — 랜덤 주행을 유지한 채 도달 시간을 줄입니다. **3회 실행 평균**으로 1단계와 비교합니다

**2단계의 두 경로** — ⓐ부터 시작하고, 여력이 있으면 ⓑ로 나아갑니다.

| 경로 | 방법 |
|:--:|------|
| **ⓐ 파라미터 조정** | 5장에서 선언한 값을 **실행 중 바꾸며 실험**(`ros2 param set`) — 주행 속도 · 조향 범위 · 후진 시간 · 회전 각도 범위 · 감지 여유. 코드 구조는 그대로 |
| **ⓑ 조향 방식 개선** | 무작위의 **분포 자체**를 바꿉니다 — 목표 방향의 조향에 가중치를 두기 · 회피 직후 진행 방향을 벽 반대쪽으로 제한하기 · 직진 구간을 길게 유지하기 |

> **3회 평균으로 비교하는 이유 —** 랜덤 주행은 같은 코드로도 실행마다 결과가 다릅니다. 한 번 빨랐다는 것은 알고리즘이 개선되었다는 근거가 되지 않습니다. 확률적으로 동작하는 방법은 여러 번의 평균으로 평가합니다 — Day 5의 분류 정확도, Day 9의 회피 성공률도 같은 방식으로 판단합니다.

<details>
<summary><b>1단계 정답 및 풀이 보기</b></summary>
...
class MazeDriver(Node):
    def __init__(self):
        ...
        self.tick = 0
        self.t0 = self.get_clock().now()   # 출발 시각 — 도달 시간 측정용
        self.done = False                  # 도달 후 로그 1회만
    ...
    def timer_callback(self):
        ...
        if GOAL[0] < self.pose.x < GOAL[1] and GOAL[2] < self.pose.y < GOAL[3]:
            self.pub.publish(Twist())            # 1단계 도달 — 정지
            if not self.done:                    # 로그는 1회만
                sec = (self.get_clock().now() - self.t0).nanoseconds / 1e9
                self.get_logger().info(f'goal! {sec:.1f}s')
                self.done = True
            return
        msg = Twist()
        if self.state == 'RUN':
...
- 랜덤 조향·랜덤 회전의 의미 — 경로 계획 없이도 반복 시도로 언젠가 목표에 도달합니다. **2단계는 이 "언젠가"를 앞당기는 과제**입니다
- 시간 측정 — `get_clock().now()`는 노드의 현재 시각이며, 두 시각의 차는 나노초 단위이므로 `1e9`로 나눠 초로 환산합니다
- launch 일괄 실행 — 8.3의 demo_launch.py를 복사해 `maze_launch.py`를 작성하고(circle_driver 항목을 maze_driver로 교체) `ros2 launch my_first_pkg maze_launch.py`로 실행합니다

</details>

<details>
<summary><b>2단계 ⓐ — 파라미터 조정 보기</b></summary>

먼저 어느 값이 시간을 좌우하는지 확인합니다.

| 값 | 시간에 미치는 영향 |
|------|------|
| 주행 속도 | 높이면 이동은 빠르나 **벽 감지 후 후진량이 늘어** 이득이 상쇄될 수 있음 |
| 조향 범위(`uniform`의 폭) | 좁히면 직진성이 커져 멀리 이동 · 넓히면 제자리 배회가 늘어남 |
| 회전 각도 범위(`randint`) | 회피 후 진행 방향의 분포를 정함 |
| 감지 여유(0.2) | 줄이면 벽에 가까이 붙어 통로를 지나기 쉬우나 **충돌 위험** |

</details>

<details>
<summary><b>2단계 ⓑ — 조향 방식 개선 보기</b></summary>

무작위의 분포 자체를 바꿉니다.

- **목표 방향 가중** — 목표 구역 방향(현재 위치에서 목표까지의 각도)과 현재 진행 방향의 차이를 구해, 조향 난수에 그 방향의 가중치를 더합니다. 무작위성은 유지하되 **평균적으로 목표 방향으로 치우치게** 하는 방식입니다
- **회피 후 방향 제한** — TURN에서 완전 무작위 대신 **벽 반대쪽 범위**에서만 각도를 뽑습니다(같은 벽에 다시 부딪히는 왕복을 줄입니다)
- **직진 유지** — RUN에서 매 틱 조향하지 않고 일정 틱마다 조향해 직진 구간을 길게 가져갑니다

측정 — 각 방법마다 **3회 실행 평균**을 기록해 비교합니다. 값 하나를 바꿀 때마다 측정하는 것이 아니라, **바꾼 이유와 예상을 먼저 적고** 확인하는 순서로 진행합니다.

이 구조는 **Day 9 실물 회피 주행의 성능 조정과 같습니다** — 그때는 대상이 초음파 감지 거리·회피 각도로 바뀔 뿐입니다.
...
| 옵션 | 해제하면 보이는 것 |
|------|------|
| Dead sinks | 구독자가 없는 토픽 — 발행만 되고 수신하는 노드가 없는 상태 |
...
| ⑤ | **원격 연결** — `openssh-server` 설치 + 원격 데스크톱 켬(설정 → 시스템) | PC에서 원격 데스크톱 연결 성공 |
...
| 확인 | 정상 |
|------|------|
| PC의 **원격 데스크톱 연결**(RDP, Remote Desktop Protocol)로 연결 | RPi5 바탕화면이 PC 화면에 표시 |
| `ssh 사용자명@주소` | 터미널 연결 |
| RPi5에서 `hostname -I` | 주소 출력 — **메모해 둘 것** |

- 원격 데스크톱 연결 이름·암호 = RPi5 원격 데스크톱 설정에 표시된 값(Ubuntu 로그인 계정과 별도 — 상세 Day 4 자료 3.2)
...
- 다만 카메라 실습이 지연되므로 기한 내 완료를 권합니다
...
| 미로 자율주행 | 가상 벽(좌표 판정) + 상태 기계(RUN/BACK/TURN) — **인식 → 판단 → 행동** 결합. **1단계 = 목표 도달·완주 / 2단계 = 도달 시간 단축**(3회 평균으로 비교) |
| 이후 연결 | 미로의 뼈대 = Day 8 판단 노드·Day 9 실물 회피와 동일 — 센서만 Pose → 카메라·초음파로 교체 |
