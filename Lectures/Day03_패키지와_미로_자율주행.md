...
| **ⓑ 조향 방식 개선** | 무작위의 **분포 자체**를 바꿉니다 — 목표 방향 쪽 조향에 가중치를 두기 · 회피 직후 진행 방향을 벽 반대쪽으로 제한하기 · 직진 구간을 길게 유지하기 |

> **3회 평균으로 비교하는 이유 —** 랜덤 주행은 같은 코드로도 실행마다 결과가 다릅니다. 한 번 빨랐다는 것은 알고리즘이 좋아졌다는 근거가 되지 않습니다. 확률적으로 동작하는 방법은 여러 번의 평균으로 평가합니다 — Day 5의 분류 정확도, Day 9의 회피 성공률도 같은 방식으로 판단합니다.
...
- **목표 방향 가중** — 목표 구역 방향(현재 위치에서 목표까지의 각도)과 현재 진행 방향의 차이를 구해, 조향 난수에 그 방향의 가중치를 더합니다. 무작위성은 유지하되 **평균적으로 목표 쪽으로 치우치게** 하는 방식입니다
...
측정 — 각 방법마다 **3회 실행 평균**을 기록해 비교합니다. 값 하나를 바꿀 때마다 재는 것이 아니라, **바꾼 이유와 예상을 먼저 적고** 확인하는 순서로 진행합니다.
