...
| 이후 일정 | Day 6·9~12 실물 구간은 차량에 실린 RPi5에서 동작(Day 7 SLAM 실습만 PC) — 지금 옮겨 두면 전환이 수월함 |
...
> **오늘 놓치지 말아야 할 것 —** 카메라가 구동되지 않아도 **Day 5의 영상 처리 원리와 노드 코드는 그대로 학습**할 수 있습니다. 환경은 뒤에 복구하면 되지만, 카메라에서 영상이 출력되는 상태는 **Day 5 이후의 전제**입니다.
...
4.1·5.1의 조치를 끝까지 수행해도 카메라가 구동되지 않아 **교수가 전환을 안내한 경우**에 사용합니다. 실패에 대비해 별도 경로를 준비해 두는 같은 방식을 Day 6 결선·Day 9 실물 전환에서도 사용합니다.
